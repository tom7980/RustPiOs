...
impl MiniUart {
    ...
    pub fn init(&mut self) {
        ...
        let mut _pin14 = GpioPin::new(14).into_alt(Function::Alt5);
        _pin14.set_no_pud();

        let mut _pin15 = GpioPin::new(15).into_alt(Function::Alt5);
        _pin15.set_no_pud();

        self.registers.lcr.set(0b11);

        let divisor = ((250000000/9600)/8)- 1;
        self.registers.baud.write(BAUD::RATE.val(divisor as u32));
        self.registers.cntl.modify(CNTL::RXENABLE::SET + CNTL::TXENABLE::SET);
    }

    pub fn write_byte(&mut self, byte: u8) {
        while !self.registers.lsr.matches_any(LSR::TXEMPTY::SET) {};
        self.registers.io.set((0 | byte) as u32);
    }
...
register_structs!{
    MiniRegisters{
        ...
        (0x008 => _r2),
        (0x040 => io: ReadWrite<u32, IO::Register>),
